...
namespace TinyFramework
{
	...
	//////////////////////////////////////////////////////////////////////////
	TinyStringMap::TinyStringMap(INT_PTR blockSize)
	...
	TinyStringMap::~TinyStringMap()
	{
		RemoveAll();
	}
	INT_PTR	TinyStringMap::GetSize() const
	{
		return m_count;
	}
	BOOL TinyStringMap::IsEmpty() const
	{
		return m_count == 0;
	}
	void TinyStringMap::Initialize(UINT hashSize)
	{
		if (hashSize == 0)
			hashSize = 17;
	...
	UINT TinyStringMap::HashKey(LPCTSTR key) const
	{
		//STLÀ„∑®xfunctional
	...
	BOOL TinyStringMap::Lookup(LPCTSTR key, void*& value) const
	{
		UINT bucket = 0;
		UINT hash = 0;
		TinyNode* ps = NULL;
	...
	void*& TinyStringMap::operator[](LPCTSTR key)
	{
		UINT bucket = 0;
		UINT hash = 0;
		TinyNode* ps = NULL;
	...
	void TinyStringMap::SetAt(LPCTSTR key, void* value)
	{
		(*this)[key] = value;
	}
	BOOL TinyStringMap::Remove(LPCTSTR key)
	{
		if (m_pTable == NULL)
			return FALSE;
	...
	TinyStringMap::TinyNode* TinyStringMap::New()
	{
		if (m_pFreeList == NULL)
		{
...
#undef new
		new(&pNew->m_key) TinyString;
#pragma pop_macro("new")
		pNew->m_value = NULL;
		return pNew;
	}
	void TinyStringMap::Delete(TinyNode* ps)
	{
		DestructElement(&ps->m_key);
		ps->m_pNext = m_pFreeList;
		m_pFreeList = ps;
	...
	TinyStringMap::TinyNode* TinyStringMap::Lookup(LPCTSTR key, UINT& bucket, UINT& hash) const
	{
		hash = HashKey(key);
