...
#define MIN_BUFF_SIZE (1 << 18)
	...
	static void* tiny_copy(void* d, const void* s, size_t size, my_memcpy mymemcpy)
	{
		if (!d || !s)
			return NULL;
	...
	{
		return tiny_copy(d, s, size, memcpy);
	}
	void* io_gpu_memcpy(void* d, const void* s, size_t size)
	{
		return (g_AVX2_enabled) ?
			tiny_copy(d, s, size, gpu_memcpy_avx2) :
			tiny_copy(d, s, size, gpu_memcpy_sse41);
	}
