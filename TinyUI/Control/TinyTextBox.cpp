...
namespace TinyUI
{
	IMPLEMENT_DYNAMIC(TinyTextBox, TinyControl);
	...
	{
		return (WS_EX_LEFT | WS_EX_CLIENTEDGE);
	}
	...
	TinyString TinyTextBox::GetText()
	{
		ASSERT(::IsWindow(m_hWND));
		INT iRes = ::GetWindowTextLength(m_hWND);
		TinyString str(iRes + 1);
		::GetWindowText(m_hWND, str.STR(), MAX_PATH);
		return str;
	}
