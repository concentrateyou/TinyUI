...
namespace TinyUI
{
	namespace Media
	{
		...
		BOOL TinyXAudio::Start()
		{
			...
			if (!m_pSourceVoice)
				return FALSE;
			HRESULT hRes = m_pSourceVoice->FlushSourceBuffers();
			if (FAILED(hRes))
			{
				LOG(ERROR) << "[TinyXAudio] [Start] FlushSourceBuffers:" << hRes;
			}
			hRes = m_pSourceVoice->Start(0, XAUDIO2_COMMIT_NOW);
			if (FAILED(hRes))
			{
