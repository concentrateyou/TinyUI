...
namespace DXFramework
{
	...
	HRESULT DX11CreateTextureFromWIC(ID3D11Device* d3dDevice,
	...
	HRESULT DX10CreateTextureFromWIC(ID3D10Device1* d3dDevice,
		IWICBitmapFrameDecode *frame,
		size_t maxsize,
		D3D10_USAGE usage,
		UINT bindFlags,
		...
		ID3D10ShaderResourceView** textureView)
	{
		...
		if ((format == DXGI_FORMAT_R32G32B32_FLOAT) && d3dContext != 0 && textureView != 0)
		{
			UINT fmtSupport = 0;
			hRes = d3dDevice->CheckFormatSupport(DXGI_FORMAT_R32G32B32_FLOAT, &fmtSupport);
			if (FAILED(hRes) || !(fmtSupport & D3D10_FORMAT_SUPPORT_MIP_AUTOGEN))
			{
				memcpy(&convertGUID, &GUID_WICPixelFormat128bppRGBAFloat, sizeof(WICPixelFormatGUID));
				format = DXGI_FORMAT_R32G32B32A32_FLOAT;
				bpp = 128;
		...
		UINT support = 0;
		hRes = d3dDevice->CheckFormatSupport(format, &support);
		if (FAILED(hRes) || !(support & D3D10_FORMAT_SUPPORT_TEXTURE2D))
		{
			memcpy(&convertGUID, &GUID_WICPixelFormat32bppRGBA, sizeof(WICPixelFormatGUID));
			format = DXGI_FORMAT_R8G8B8A8_UNORM;
			bpp = 32;
	...
	HRESULT DX11CreateWICTextureFromMemory(ID3D11Device* d3dDevice,
		const BYTE* wicData,
		size_t wicDataSize,
		ID3D11Resource** texture,
		ID3D11ShaderResourceView** textureView,
		size_t maxsize)
	{
		return DX11CreateWICTextureFromMemoryEx(d3dDevice, wicData, wicDataSize, maxsize,
			D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, 0, D3D11_RESOURCE_MISC_SHARED, WIC_LOADER_DEFAULT,
			texture, textureView);
	}


	HRESULT DX11CreateWICTextureFromMemoryEx(ID3D11Device* d3dDevice,
		ID3D11DeviceContext* d3dContext,
		const BYTE* wicData,
		size_t wicDataSize,
		size_t maxsize,
		...
		ID3D11ShaderResourceView** textureView)
	{
		...
		if (hRes != S_OK)
			return hRes;
		hRes = DX11CreateTextureFromWIC(d3dDevice, d3dContext,
			frame, maxsize,
		...
		ID3D11ShaderResourceView** textureView)
	{
		...
		if (hRes != S_OK)
			return hRes;
		hRes = DX11CreateTextureFromWIC(d3dDevice, NULL,
			frame, maxsize,
			usage, bindFlags, cpuAccessFlags, miscFlags, loadFlags,
			texture, textureView);
		if (hRes != S_OK)
	...
	HRESULT DX11CreateWICTextureFromMemory(ID3D11Device* d3dDevice,
		...
		size_t maxsize)
	{
		return DX11CreateWICTextureFromMemoryEx(d3dDevice, d3dContext, wicData, wicDataSize, maxsize,
			D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, 0, D3D11_RESOURCE_MISC_SHARED, WIC_LOADER_DEFAULT,
			texture, textureView);
	}


	HRESULT DX11CreateWICTextureFromFile(ID3D11Device* d3dDevice,
		ID3D11DeviceContext* d3dContext,
		...
		size_t maxsize)
	{
		return DX11CreateWICTextureFromFileEx(d3dDevice, d3dContext, fileName, maxsize,
			D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, 0, D3D11_RESOURCE_MISC_SHARED, WIC_LOADER_DEFAULT,
			texture, textureView);
	}
	...
	HRESULT DX11CreateWICTextureFromFile(ID3D11Device* d3dDevice,
		...
		size_t maxsize)
	{
		return DX11CreateWICTextureFromFileEx(d3dDevice, fileName, maxsize,
			D3D11_USAGE_DEFAULT, D3D11_BIND_SHADER_RESOURCE, 0, D3D11_RESOURCE_MISC_SHARED, WIC_LOADER_DEFAULT,
			texture, textureView);
	}


	HRESULT DX11CreateWICTextureFromFileEx(ID3D11Device* d3dDevice,
		const WCHAR* fileName,
		size_t maxsize,
		D3D11_USAGE usage,
		UINT bindFlags,
		...
		ID3D10ShaderResourceView** textureView)
	{
		...
		if (hRes != S_OK)
			return hRes;
		hRes = DX10CreateTextureFromWIC(d3dDevice,
			frame, maxsize,
			usage, bindFlags, cpuAccessFlags, miscFlags, loadFlags,
			texture, textureView);
		if (hRes != S_OK)
