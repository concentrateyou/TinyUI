...
{
	DX10YUVBT709Shader::DX10YUVBT709Shader()
	{
	}
	...
	BOOL DX10YUVBT709Shader::Initialize(DX10& dx10, const CHAR* vsFile, const CHAR* psFile)
	{
		HRESULT hRes = S_OK;
		...
		hRes = D3DCompileFromFile(StringToWString(vsFile).c_str(), NULL, NULL, "YUVBT709VertexShader", "vs_4_0_level_9_3", D3DCOMPILE_ENABLE_STRICTNESS, 0, &vertexShaderBuffer, &errorMsg);
		if (hRes != S_OK)
		{
			CHAR* error = (CHAR*)errorMsg->GetBufferPointer();
			LOG(ERROR) << "DX10TextureShader Initialize - YUVBT709VertexShader: " << error;
			return FALSE;
		}
		hRes = D3DCompileFromFile(StringToWString(psFile).c_str(), NULL, NULL, "YUVBT709PixelShader", "ps_4_0_level_9_3", D3DCOMPILE_ENABLE_STRICTNESS, 0, &pixelShaderBuffer, &errorMsg);
		if (hRes != S_OK)
		{
			CHAR* error = (CHAR*)errorMsg->GetBufferPointer();
			LOG(ERROR) << "DX10TextureShader Initialize - YUVBT709PixelShader: " << error;
			return FALSE;
		}
	...
	void DX10YUVBT709Shader::Render(DX10& dx10)
	{
	...
	void DX10YUVBT709Shader::SetShaderParameters(DX10& dx10, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, DX10Texture2D* textures[2])
	{
		...
		ID3D10ShaderResourceView* spv1 = textures[0]->GetSRView();
		ASSERT(spv1);
		ID3D10ShaderResourceView* spv2 = textures[1]->GetSRView();
		ASSERT(spv2);
		ID3D10ShaderResourceView* spv3 = textures[2]->GetSRView();
		ASSERT(spv3);
		dx10.GetD3D()->PSSetShaderResources(0, 2, &spv1);
		dx10.GetD3D()->PSSetShaderResources(1, 2, &spv2);
		dx10.GetD3D()->PSSetShaderResources(2, 2, &spv3);
	}
}