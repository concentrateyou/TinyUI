...
namespace DXFramework
{
	...
	BOOL DX11TransparentShader::Initialize(DX11& dx11, const CHAR* vsFile, const CHAR* psFile)
	{
		...
		hRes = D3DCompileFromFile(wvsFile.c_str(), NULL, NULL, "TransparentVertexShader", "vs_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &vertexShaderBuffer, NULL);
		if (hRes != S_OK)
			return FALSE;
		wstring wpsFile = StringToWString(psFile);
		hRes = D3DCompileFromFile(wpsFile.c_str(), NULL, NULL, "TransparentPixelShader", "ps_4_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, &pixelShaderBuffer, NULL);
		if (hRes != S_OK)
			return FALSE;
