...
namespace Encode
{
	x264Encoder::x264Encoder(Callback<void(BYTE*, LONG, const MediaTag&)>&& callback)
		:m_x264Image(NULL),
		m_x264(NULL),
		m_x264Param(NULL),
		m_sINC(0),
	...
	BOOL x264Encoder::Open(INT cx, INT cy, INT videoFPS, INT videoRate)
	{
		Close();
	...
	BOOL x264Encoder::BuildParam(INT cx, INT cy, INT videoFPS, INT videoRate)
	{
	...
	BOOL x264Encoder::Encode(AVFrame* pI420)
	{
